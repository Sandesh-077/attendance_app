...
class _WebAdminAppState extends State<WebAdminApp> {
  ...
  late final GoRouter _router = GoRouter(
    ...
    routes: [
      ...
      GoRoute(path: '/admin', builder: (_, _) => _AdminPage(auth: _auth)),
      GoRoute(
        path: '/access-denied',
...
class _LoginPageState extends State<_LoginPage> {
  ...
  Future<void> _login() async {
    ...
    final error = await _repository.login(email: _email.text, password: _password.text);
  ...
  Future<void> _resetPassword() async {
    ...
    try {
      ...
      if (mounted) setState(() => _error = 'Unable to send a reset email. Check the address and try again.');
    }
  }
        ...
        child: ConstrainedBox(
          ...
          child: Form(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              Text('CA Attendance', textAlign: TextAlign.center, style: Theme.of(context).textTheme.headlineMedium),
              const SizedBox(height: 8),
              const Text('Admin Web sign in', textAlign: TextAlign.center),
              const SizedBox(height: 28),
              TextField(controller: _email, enabled: !_busy, keyboardType: TextInputType.emailAddress,
                autofillHints: const [AutofillHints.email], textInputAction: TextInputAction.next,
                decoration: const InputDecoration(labelText: 'Email', border: OutlineInputBorder())),
              const SizedBox(height: 16),
              TextField(controller: _password, enabled: !_busy, obscureText: _hidden,
                onSubmitted: (_) => _login(),
                decoration: InputDecoration(labelText: 'Password', border: const OutlineInputBorder(),
                  suffixIcon: IconButton(onPressed: () => setState(() => _hidden = !_hidden),
              ...
              Align(alignment: Alignment.centerRight, child: TextButton(onPressed: _busy ? null : _resetPassword,
...
class _AdminPage extends StatelessWidget {
  const _AdminPage({required this.auth});
  final WebAuthState auth;

  @override
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('CA Attendance · Admin Web'),
      actions: [TextButton.icon(onPressed: auth.signOut, icon: const Icon(Icons.logout), label: const Text('Logout'))]),
    body: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
      Text('Welcome back, ${auth.name ?? 'Admin'}', style: Theme.of(context).textTheme.headlineSmall),
      const SizedBox(height: 12),
      const Text('Web Admin dashboard is being prepared.'),
    ])),
  );
}
  ...
  Widget build(BuildContext context) => Scaffold(
    body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(
      mainAxisSize: MainAxisSize.min, children: [
        Text('Access unavailable', style: Theme.of(context).textTheme.headlineMedium),
        const SizedBox(height: 12),
        const Text('This account does not currently have access to CA Attendance Web.', textAlign: TextAlign.center),
        const SizedBox(height: 20),
        FilledButton.icon(onPressed: auth.signOut, icon: const Icon(Icons.logout), label: const Text('Logout')),
      ],
    ))),
  );
}
