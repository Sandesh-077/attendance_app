...
enum WebAuthStatus { loading, signedOut, admin, unsupported, missingProfile, profileError }
...
class WebAuthState extends ChangeNotifier {
  WebAuthState({FirebaseAuth? auth, FirebaseFirestore? firestore})
    ...
    _authSubscription = _auth.authStateChanges().listen(_onUser, onError: (_) {
    ...
    _profileSubscription = _firestore.collection('users').doc(user.uid).snapshots().listen(
      (snapshot) {
        ...
        name = profileName is String && profileName.trim().isNotEmpty
            ? profileName.trim()
            : null;
        _setStatus(role == 'Admin' ? WebAuthStatus.admin : WebAuthStatus.unsupported);
      },
      onError: (_) {
        if (generation == _generation) _setStatus(WebAuthStatus.profileError);
      },
    );
  }
